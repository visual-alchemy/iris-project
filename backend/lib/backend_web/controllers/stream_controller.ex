defmodule BackendWeb.StreamController do
  ...
  def server_stats(conn, _params) do
    ...
    total_connections =
      Enum.reduce(mediamtx_paths, 0, fn path, acc ->
        acc + (Map.get(path, "readers", []) |> length()) +
          (Map.get(path, "readers", []) |> length())
      end)
  ...
  def database_stats(conn, _params) do
    ...
    db_size =
      try do
        result =
          Backend.Repo.query!("SELECT pg_size_pretty(pg_database_size(current_database()))")
